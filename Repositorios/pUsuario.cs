    ...
    public class pUsuario : RepositorioBase, IpUsuario
    {
        ...
        public ColecaoUsuario Consultar(dUsuario dados)
        {
            try
            {
                using (var conn = CriarConexao())
                {
                    ...
                    if (dados.cid.HasValue && dados.cid != 0) { conditions.Add("u.cid=@cid"); p.Add("cid", dados.cid); }
                    ...
                    if (dados.usuarioPerfil_cid.HasValue && dados.usuarioPerfil_cid != 0) { conditions.Add("u.usuarioPerfil_cid=@usuarioPerfil_cid"); p.Add("usuarioPerfil_cid", dados.usuarioPerfil_cid); }
                    if (dados.descontoProduto.HasValue && dados.descontoProduto != 0) { conditions.Add("u.descontoProduto=@descontoProduto"); p.Add("descontoProduto", dados.descontoProduto); }
                    if (dados.descontoPedido.HasValue && dados.descontoPedido != 0) { conditions.Add("u.descontoPedido=@descontoPedido"); p.Add("descontoPedido", dados.descontoPedido); }
                    if (dados.comissao.HasValue && dados.comissao != 0) { conditions.Add("u.comissao=@comissao"); p.Add("comissao", dados.comissao); }
        ...
        public int Alterar(dUsuario dados)
        {
            try
            {
                using (var conn = CriarConexao())
                {
                    return conn.Execute("UPDATE usuarios SET usuario=@usuario, senha=CASE WHEN @senha IS NULL OR @senha = '' THEN senha ELSE @senha END, nomeCompleto=@nomeCompleto, situacao=@situacao, descontoProduto=@descontoProduto, descontoPedido=@descontoPedido, comissao=@comissao, usuarioPerfil_cid=@usuarioPerfil_cid, email=@Email WHERE cid=@cid",
