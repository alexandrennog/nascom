            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow linha in dt.Rows)
                    {
                        ...
                        item.cod_part = Comum.cFuncoes.RetornarTexto(linha["cid"]);
                        item.nome = Comum.cFuncoes.RetornarTexto(linha["nome"]);
                        item.cnpj = Comum.cFuncoes.RetornarTexto(linha["cnpj"]);
                        item.ie = Comum.cFuncoes.RetornarTexto(linha["inscricaoEstadual"]);
                        item.cod_mun = Comum.cFuncoes.RetornarTexto(linha["codigo_ibge"]);
                        item.ende = Comum.cFuncoes.RetornarTexto(linha["logradouro"]);
                        item.num = Comum.cFuncoes.RetornarTexto(linha["numero"]);
                        item.compl = Comum.cFuncoes.RetornarTexto(linha["complemento"]);
                        item.bairro = Comum.cFuncoes.RetornarTexto(linha["bairro"]);
                        retorno.Add(item);
                    }
                }
            }
