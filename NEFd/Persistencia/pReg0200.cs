            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow linha in dt.Rows)
                    {
                        ...
                        item.cod_item = Comum.cFuncoes.RetornarTexto(linha["codigo"]);
                        item.descr_item = Comum.cFuncoes.RetornarTexto(linha["descricao"]);
                        item.aliq_icms = Comum.cFuncoes.RetornarTexto(linha["aliquotaIcms"]);
                        item.unid_inv = Comum.cFuncoes.RetornarTexto(linha["efdUnidadeMedidaCodigo"]);
                        retorno.Add(item);
                    }
                }
            }
