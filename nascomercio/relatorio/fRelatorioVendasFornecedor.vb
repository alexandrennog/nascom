...
Public Class fRelatorioVendasFornecedor
    ...
    Private Sub CarregarDados()
        Me.nascomercioDataSet.v_vendasfornecedor.Clear()
        ...
        Using conexao As New MySql.Data.MySqlClient.MySqlConnection(Global.nascomercio.My.MySettings.Default.nascomercioConnectionString)
            Dim sql As String = "SELECT `codigo`, `produto`, `referencia`, `cor`, `fornecedor`, `cd_fornecedor`, `controle`, `quantidade`, `valor`, `total_valor`, `data` FROM `v_vendasfornecedor` WHERE `data` BETWEEN @dataInicial AND @dataFinal"
            ...
            Using comando As New MySql.Data.MySqlClient.MySqlCommand(sql, conexao)
                ...
                If txtFornecedor.Text.Trim() <> "" Then
                    comando.Parameters.AddWithValue("@fornecedor", "%" & txtFornecedor.Text.Trim() & "%")
                End If

                Using adapter As New MySql.Data.MySqlClient.MySqlDataAdapter(comando)
                    adapter.Fill(Me.nascomercioDataSet.v_vendasfornecedor)
                End Using
            End Using
        End Using
    End Sub
    ...
    Private Sub Filtrar()
        ...
        If txtFornecedor.Text <> "" Then
            ...
            parametros(2).Values.Add(txtFornecedor.Text)
        Else
            parametros(2) = New Microsoft.Reporting.WinForms.ReportParameter
            parametros(2).Name = "Fornecedor"
        End If
    ...
    Private Sub fRelatorioVendasFornecedor_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
